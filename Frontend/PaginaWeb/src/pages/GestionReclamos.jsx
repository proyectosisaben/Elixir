...
function GestionReclamos() {
  ...
  const cargarReclamos = async () => {
    try {
      ...
      const url = `http://localhost:8000/api/reclamos/${queryString ? '?' + queryString : ''}`;
  ...
  const cargarComentarios = async (reclamoId) => {
    try {
      const response = await fetch(`http://localhost:8000/api/reclamos/${reclamoId}/comentarios/?usuario_id=${usuario?.id}`, {
        headers: {
          'Content-Type': 'application/json'
        }
      });
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = reclamoEditando 
        ? `http://localhost:8000/api/reclamos/${reclamoEditando.id}/`
        : 'http://localhost:8000/api/reclamos/';
  ...
  const handleAgregarComentario = async (e) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/reclamos/${reclamoSeleccionado.id}/comentarios/`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json'
        },
  ...
  const handleEliminar = async (reclamoId) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/reclamos/${reclamoId}/?usuario_id=${usuario?.id}`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json'
        }
